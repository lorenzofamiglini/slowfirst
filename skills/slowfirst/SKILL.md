...
4. **Steps.** Propose the smallest approach first: use, configure or wrap what exists before building anything new. Break it into steps that are each small enough to review and can be verified on their own. The human chooses.
...
- Verify each step by running or testing it before starting the next, then tick it in the brief.
